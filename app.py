...
def card(title, value, sub=""):
    st.markdown(
        f'<div class="card"><div class="muted">{title}</div>'
        f'<h2>{value}</h2><div class="muted">{sub}</div></div>',
...
def show_result(r):
    ...
    with a:
        card("Indicator", r["indicator"])
    with b:
        card("Type", r["indicator_type"])
    with c:
        card("Threat Score", r["score"] if r["score"] is not None else "Unknown", r["label"])
    with d:
        card("Evidence Coverage", r["coverage"], "Provider availability")

    if r["score"] is not None:
        st.progress(max(0, min(100, r["score"])) / 100)

    if r["conflicts"]:
        st.warning("Conflicting evidence detected: " + "; ".join(r["conflicts"]))
    ...
    for col, name, key in zip(
        ...
        with col:
            ...
            if data:
                ...
                card(name, "Evidence available", summary or "")
            else:
                card(name, "Unavailable / No result")

    st.subheader("Security Signals")
    if r["signals"]:
        st.dataframe(r["signals"], use_container_width=True, hide_index=True)
    else:
...
if page == "Scanner":
    st.markdown(
        '<div class="hero"><h1>🛡️ Cyber Sentinel AI</h1>'
        '<p>Intelligent Threat Detection. Safer Digital Decisions.</p></div>',
        unsafe_allow_html=True,
    )
    ...
    if kind == "URL":
        st.warning(
            "URLScan submission shares the URL with URLScan. Never submit credentials, "
            "tokens, private URLs, or sensitive data."
        )
    ...
    st.markdown(
        '<div class="hero"><h1>🛡️ Scan History</h1>'
        '<p>Session-only results.</p></div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="hero"><h1>🛡️ About Cyber Sentinel AI</h1>'
        '<p>Intelligent Threat Detection. Safer Digital Decisions.</p></div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        "Cyber Sentinel AI combines local validation with optional VirusTotal, "
        "AbuseIPDB, URLScan and Gemini evidence interpretation."
    )
    ...
    st.markdown(
        '<div class="hero"><h1>⚙️ Settings</h1>'
        '<p>Integration status. Secret values are never displayed.</p></div>',
        unsafe_allow_html=True,
    )
    ...
    for col, name, key in zip(
        ...
        with col:
            card(name, "Connected" if av[key] else "Not configured")
    st.caption(
        "Keys are read from Streamlit secrets first and environment variables second. "
        "Expected names: VT_API_KEY, ABUSEIPDB_API_KEY, URLSCAN_API_KEY, GEMINI_API_KEY."
    )
