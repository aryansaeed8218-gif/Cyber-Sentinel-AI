...
def gemini_analysis(result):
    """
    Generate the AI interpretation using Gemini's REST API directly.

    This intentionally avoids importing the Google GenAI Python SDK, so the
    Gemini feature does not depend on a separate google-genai package being
    installed in the Streamlit deployment. The existing GEMINI_API_KEY secret
    ...
    try:
        ...
        # Gemini can occasionally take longer than the other providers.
        # Retry only network timeouts/connection failures; keep the existing
        # UI and all other provider timeouts unchanged.
        response = None
        ...
        if response is None and last_network_error is not None:
